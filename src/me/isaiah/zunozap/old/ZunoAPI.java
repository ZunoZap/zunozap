...
import me.isaiah.zunozap.old.FileManager;
...
class ZunoAPI_DEPRECATED {
	 ...
	 * @since 1.0-DevTest
	 * */
    public static void checkIfExists(File file){if(file.exists()){try{file.createNewFile();}catch(IOException e){e.printStackTrace();}}}
	 ...
	 * @deprecated
	 * */
    @SuppressWarnings("unused") public static void setOnActions(Stage stage, TextField addressBar, Button googleBar, Button GOButton, Button BackButton, Button ForwardButton, Button enableTools) {
	...
	@SuppressWarnings("unused") public static void loadPageFromAddressBar(Stage stage, TextField addressBar) {
	...
	public static void loadPagePGL(Stage stage, String url, TextField addressBar) {
    	ZunoAPI_DEPRECATED.loadPage(url);
        stage.setTitle("ZunoZap " + Main.version +" - "+ WebEngine.getTitle() + " (" + WebEngine.getLocation() + ")");
	}
    @Deprecated
    public static void DownloadPage(String site, @SuppressWarnings("unused") TextField addressBar, File DPfolder) {ZunoAPI_DEPRECATED.DownloadPage(site, DPfolder);}
    ...
    public static void HPLS() {
       	for (int i = 0; i < ImgButtons.captions.length; i++) {
        	...
        	hpl.setOnAction(new EventHandler<ActionEvent>() {@Override public void handle(ActionEvent e) {
        		ZunoAPI_DEPRECATED.loadPage(url); ZunoAPI_DEPRECATED.addressBar.setText(url); }
        	});
        }
    }
	public static void googlePage(String text) {
		ZunoAPI_DEPRECATED.loadPage("https://www.google.com/?gws_rd=ssl#q="+ text.replaceAll(" ", "+"));
	}
}
