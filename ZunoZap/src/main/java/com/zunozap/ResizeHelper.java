...
public class ResizeHelper {
    ...
    public static void addResizeListener(Stage stage, double minWidth, double minHeight, double maxWidth, double maxHeight) {
        ...
        for (Node child : children) {
            isScrollbar = child instanceof ScrollBar;
            if (!isScrollbar) addListenerDeeply(child, resizeListener);
        }
    }
    ...
    private static void addListenerDeeply(Node node, EventHandler<MouseEvent> listener) {
        ...
        if (node instanceof Parent) {
            ...
            for (Node child : children) {
                isScrollbar = child instanceof ScrollBar;
                if (!isScrollbar) addListenerDeeply(child, listener);
            }
        }
    }
    ...
    static class ResizeListener implements EventHandler<MouseEvent> {
        ...
        private void setStageWidth(double width) {
            stage.setWidth(Math.max(Math.min(width, maxWidth), minWidth));
        }

        private void setStageHeight(double height) {
            stage.setHeight(Math.max(Math.min(height, maxHeight), minHeight));
        }
    }
